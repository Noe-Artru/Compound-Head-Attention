...
def scaled_dot_product_GQA(Q, K, V, is_causal=False):
    """
...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        # Unfolding heads
        Q = rearrange(Q, "b n (h d) -> b n h d", h=self.query_heads) # d should be the same as others since Q_proj has a larger embed_dim
        K = rearrange(K, "b n (h d) -> b n h d", h=self.kv_heads)
        V = rearrange(V, "b n (h d) -> b n h d", h=self.kv_heads)

        x = scaled_dot_product_GQA(Q, K, V, is_causal=self.is_causal)
