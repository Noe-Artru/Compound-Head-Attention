...
from CHA import CompoundHeadAttention
from torchscale.component.xpos_relative_position import XPOS

class CHAEncoderLayer(nn.Module):
    def __init__(self, embed_dim, query_heads, kv_heads, feedforward_dim, layer_norm_eps = 1e-5, device=None, dtype=None):
        ...
        self.CHA_attention = CompoundHeadAttention(embed_dim, query_heads, kv_heads, is_causal=False, layer_norm=True, device=device, dtype=dtype)
    ...
    def _CHA_block(self, x):
        x = self.norm1(x)
        x = self.CHA_attention(x, x, x)
        x = self.dropout(x)
        return x
    ...
    def forward(self, x_in):
        x = x_in
        x = x + self._CHA_block(x)
        x = x + self._FF_block(x)
        return x
    
class CHADecoderLayer(nn.Module):
    def __init__(self, embed_dim, query_heads, kv_heads, feedforward_dim, layer_norm_eps = 1e-5, device=None, dtype=None):
        super().__init__()
        ...
        # CHA Block
        self.norm1 = nn.LayerNorm(embed_dim, eps=layer_norm_eps, device=device, dtype=dtype)
        self.CHA_attention = CompoundHeadAttention(embed_dim, query_heads, kv_heads, is_causal=True, layer_norm=False, device=device, dtype=dtype)

        # CHA Cross Block
        self.norm2 = nn.LayerNorm(embed_dim, eps=layer_norm_eps, device=device, dtype=dtype)
        self.CHA_attention_decoder = CompoundHeadAttention(embed_dim, query_heads, kv_heads, is_causal=True, layer_norm=True, device=device, dtype=dtype)
    ...
    def _CHA_cross_block_(self, x, memory):
        x = self.norm2(x)
        x = self.CHA_attention_decoder(x, memory, memory)
        x = self.dropout(x)
        return x
    ...
    def forward(self, x_in, memory):
        x = x_in
        x = x + self._CHA_block(x)
        x = x + self._CHA_cross_block_(x, memory)
        x = x + self._FF_block(x)
        return x
    

class CHATransformer(nn.Module):
    def __init__(self, embed_dim, query_heads, kv_heads, num_encoder_layers, num_decoder_layers, feedforward_dim, layer_norm_eps = 1e-5, device=None, dtype=None):
        ...
        self.encoder = nn.TransformerEncoder(
            encoder_layer=CHAEncoderLayer(embed_dim=embed_dim, query_heads=query_heads, kv_heads=kv_heads, feedforward_dim=feedforward_dim, layer_norm_eps=layer_norm_eps, device=device, dtype=dtype),
        ...
        self.decoder = nn.TransformerDecoder(
            decoder_layer=CHADecoderLayer(embed_dim=embed_dim, query_heads=query_heads, kv_heads=kv_heads, feedforward_dim=feedforward_dim, layer_norm_eps=layer_norm_eps, device=device, dtype=dtype),
...
class CHATransformerNextWord(nn.Module):
    def __init__(self, num_tokens_enc, num_tokens_dec, embed_dim, query_heads, kv_heads, num_encoder_layers, num_decoder_layers, feedforward_dim, layer_norm_eps = 1e-5, device=None, dtype=None):
        ...
        self.transformer = CHATransformer(embed_dim, query_heads, kv_heads, num_encoder_layers, num_decoder_layers, feedforward_dim, layer_norm_eps, device=device, dtype=dtype)
...
if __name__ == "__main__":
    ...
    model = CHATransformerNextWord(num_tokens, embed_dim=512, query_heads=8, kv_heads=2, num_encoder_layers=6, num_decoder_layers=6, feedforward_dim=2048)
